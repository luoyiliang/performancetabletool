...
class DataFrameDialog(QDialog):
    ...
    def optimize_column_widths(self, df):
        ...
        # 为 min、nom、max 列创建一个特殊的宽度组
        special_columns = ['min', 'nom', 'max']
        special_max_width = 0
        ...
        for col in range(self.table_widget.columnCount()):
            ...
            if column_name in special_columns:
                special_max_width = max(special_max_width, max_width)
            else:
                # 设置最小和最大宽度
        ...
        # 为特殊列设置统一的宽度，但限制最大宽度
        special_max_width = max(50, min(special_max_width, 100))  # 最小50，最大100
        for col in range(self.table_widget.columnCount()):
            if df.columns[col] in special_columns:
                self.table_widget.setColumnWidth(col, special_max_width)

        # 调整合并单元格的宽度
        self.adjust_merged_cell_widths()

    def adjust_merged_cell_widths(self):
        for row in range(2):  # 检查前两行（合并的单元格）
            col = 1  # 从第二列开始，因为第一列是 "File Name"
            while col < self.table_widget.columnCount():
                item = self.table_widget.item(row, col)
                if item and self.table_widget.columnSpan(row, col) > 1:
                    span = self.table_widget.columnSpan(row, col)
                    total_width = sum(self.table_widget.columnWidth(col + i) for i in range(span))
                    for i in range(span):
                        self.table_widget.setColumnWidth(col + i, total_width // span)
                    col += span
                else:
                    col += 1
